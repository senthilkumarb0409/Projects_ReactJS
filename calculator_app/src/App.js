...
const initialState = {
  ...
  operation: null,
  overwrite: false, // Added to handle overwrite logic
};
...
function reducer(state, { type, payload }) {
  switch (type) {
    ...
    case ACTIONS.CLEAR:
      return initialState; // Resetting the state to the initial state
    ...
    default:
      return state; // In case an unrecognized action type is dispatched
  }
}
...
function evaluate({ currentOperand, previousOperand, operation }) {
  ...
  if (isNaN(prev) || isNaN(current)) return "Error"; // Return "Error" for invalid operations
  let computation = "";
  switch (operation) {
    case "+":
    ...
    case "/":
      ...
      break;
    default:
      return currentOperand; // Return current operand if no valid operation
  }
